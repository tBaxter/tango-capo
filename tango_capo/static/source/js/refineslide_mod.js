 ...
 */
 ...
 ;(function ($, window, document) {
	...
	function RS(elem, settings) {
		...
		this.$slides            = this.$slider.find('> *');            // Elem Arr: Slide elements
        ...
        this.$currentIndexWrapper = $('.rs-current-index');
		this.init();
	}
	...
	RS.prototype = {
        ...
        init: function () {
            ...
            this.$slideImages = this.$slides.find('img:eq(0)');
            ...
            $('.rs-current-index').html(this.settings.startSlide + 1);
            $('.rs-count').html(this.totalSlides);
        ...
        setArrows:function () {
            ...
            this.$sliderWrap.prepend(this.settings.controlsTemplate);
        ...
        setKeys: function () {
            ...
            var that = this,
                ...
                width = (100 - ((this.totalSlides) * this.settings.thumbMargin)) / this.totalSlides + '%';

            //<Wrapper to contain thumbnails
            this.$thumbWrap = $('<div class="rs-thumb-wrap" />').prependTo(this.$sliderWrap);
            ...
            this.$slides.each(function(index) {
                var $thumb = $('.rs-thumb-wrap a').eq(index),
                    $this = $(this),
                    $img = $this.find('img');

                if ($img.length > 0) {
                    $thumb.html($img.clone());
                }
                else {
                    $thumb.html('<span>' + $this.data('slide-type') + '</span>')
                    .attr('data-slide-type', $this.data('slide-type'));
                }
            });
            ...
            this.$thumbWrap.on('click', 'a', function (e) {
                ...
                that.transition(parseInt($(this).data('rs-num'), 10)); // Call transition using identifier from thumb class
            });
        },
        captions: function() {
            var that = this,
            ...
            if (!this.inProgress) {
                ...
                if (slideNum !== this.currentPlace) {
                    ...
                    $('.rs-current-index').html(this.currentPlace + 1);
                    //$('.rs-current-index').html('foo');
	...
	Transition.prototype = {
        ...
        anims: [ // Array of possible animations
            'cubeH', 'cubeV', 'fade', 'sliceH', 'sliceV', 'slideH', 'slideV', 'scale', 'blockScale', 'kaleidoscope', 'fan', 'blindH', 'blindV'
        ],
        customAnims: [],
        init: function () {
                ...
                for (var i = 0; i < cols; i++) {
                    ...
                    for (var j = 0; j < rows; j++)  {
                        ...
                        if (newRowRemainder > 0) {
                            var addRowRemainder = newRowRemainder >= rowAdd ? rowAdd : rowRemainder;
                            newRowHeight += addRowRemainder;
                            newRowRemainder -= addRowRemainder;
                        }
